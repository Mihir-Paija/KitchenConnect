...
import { getWallet, createWallet } from "@/utils/shared/walletAPI";
...
const WalletScreen = ({ navigation }) => {
  ...
  const [tiffinNameOptions, setTiffinNameOptions] = useState([
  ...
  const createDuration = () => {
    ...
    arr.push({
      label: "This Year",
      value: flag ? currentYear + 1 : currentYear,
    });
  ...
  const handleDuration = () => {
    ...
    if (duration.label === "Last 7 days") {
      ...
      if (durationSize === 6) setDurationToggle(!durationToggle);
      else {
        setDurationSize(6);
      }
  ...
  const handleInitialRecords = () => {
    ...
    for (const value of transactions) {
      ...
      if (
        duration.label === "Last 7 days" ||
        duration.label === "Last 14 days"
      ) {
  ...
  const fillBins = (map, orders, flag) => {
    for (const value of orders) {
      ...
      if (
        ...
        if (1 <= orderDay && orderDay <= 5) index = 0;
        else if (6 <= orderDay && orderDay <= 10) index = 1;
        else if (11 <= orderDay && orderDay <= 15) index = 2;
        else if (16 <= orderDay && orderDay <= 20) index = 3;
        else if (21 <= orderDay && orderDay <= 25) index = 4;
        else index = 5;
                        ...
                        */}
                        <View style={styles.duration}>
                          ...
                          <RNPickerSelect
                            ...
                            onValueChange={(label) => {
                              const selectedOption = findOptionByLabel(
                                label,
                                durationOptions
                              );
